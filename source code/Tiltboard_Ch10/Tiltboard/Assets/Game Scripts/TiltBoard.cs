...
public class TiltBoard : MonoBehaviour {
	...
	internal bool allowTilt = true; // flag for pausing tilt
...
//		transform.Rotate(vert, 0, hor,Space.World); 
		...
		float xRotation = Input.GetAxis("Vertical2")  * speed * Time.deltaTime;
		float zRotation = -Input.GetAxis("Horizontal2")   * speed * Time.deltaTime;
