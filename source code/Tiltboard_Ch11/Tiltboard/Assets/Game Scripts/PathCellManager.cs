...
public class PathCellManager : MonoBehaviour {
	...
	public GameObject finalFX; // the particle effect for the end of path

	GameHUD gameHUD; // so the timer can be turned off at game end

	public Animator buttonAni; // button's animator component
...
//	}
	...
	void LoadPath () {
		...
		// position the particle fx at the path end
		Vector3 newPos = new Vector3(finalFX.transform.position.x,finalFX.transform.position.y,finalFX.transform.position.z);
		newPos = allPathCells[pathLength-1].transform.position;
		finalFX.transform.position = newPos;
	...
	void ProcessWinner () {
		...
		persistent.CalculateWinCash(gameHUD. GetGameTime ());
		gameHUD.UpdateTipCash();
		print("Winner");
		// cue the FX
		finalFX.SetActive(true); // turn on the final effects
		StartCoroutine(Deactivate(finalFX ,2.5f)); // send the object off to be deactivated
		//stop input
	...
	IEnumerator Deactivate (GameObject target, float pause) {
		// pause before reactivation
